...
int request_marshall(buffer *b, const enum socksResponseStatus status) {
    size_t  n;
...
enum socksResponseStatus cmd_resolve(struct request* request,  struct sockaddr **originaddr,
            socklen_t *originlen, int *domain) {
    enum socksResponseStatus ret = statusGeneralSOCKSServerFailure;
...
enum socksResponseStatus errno_to_socks(const int e) {
    enum socksResponseStatus ret = statusGeneralSOCKSServerFailure;
    switch (e) {
        case 0:
            ret = statusSucceeded;
            break;
        case ECONNREFUSED:
            ret = statusConnectionRefused;
            break;
        case EHOSTUNREACH:
            ret = statusHostUnreachable;
            break;
        case ENETUNREACH:
            ret = statusNetworkUnreachable;
            break;
        case ETIMEDOUT:
            ret = statusTTLExpired;
            break;
    }
    return ret;
}